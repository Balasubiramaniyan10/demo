...
<head>
<meta http-equiv="Content-Type" content="text/html;charset=ISO-8859-1" /><%@ include file="/header2.jsp" %><title><%
	out.print(Webroutines.getShopNameFromShopId(cart.shopid,"")+" shopping recommendations on Vinopedia");

%></title>
<%
	session.setAttribute("winename", ((p.s!=null&&p.s.wineset!=null&&p.s.wineset.knownwineid>0)?Knownwines.getKnownWineName(p.s.wineset.knownwineid):p.searchdata.getName()).replaceAll("^\\d\\d\\d\\d\\d\\d ", ""));
...
<meta name="description" content="Vinopedia shows you recommendations on the best wines <%=shop.name %> has to offer." /><%@ include file="/snippets/guidedsearchincludes.jsp" %>
...
<body onload="javascript:loadframe();init();">
...
<% 	
	...
	if (rightadhtml.equals("")){
		Ad rightad=null;
		if (p.s.wineset==null){
			rightad = new Ad("winered",160, 600, p.hostcountry, "", 0,"");
		} else {
			rightad = new Ad("winered",160, 600, p.hostcountry, p.s.wineset.region, p.s.wineset.bestknownwineid,"");
		}
		rightad.getHTML();
...
%>
		...
		<div class='main<%if (skyscraper.length()>0) {%>store<%} %>' itemscope itemtype='http://schema.org/LiquorStore'><br/>
...
<div class='clearboth'></div>
		<ul class='tabs' id='storetabs'><li><a href='#storehome'>Selected wine</a></li><li><a href='#storegeneral'>General info</a></li><li><a itemprop='maps' href='#storelocation' id='maptab'>Location</a></li><li><a href='#recommendations' id='recommendationtab'>Shopping Guide</a></li><li><a href='#storestats' id='statstab'>Wine regions</a></li><li><a href='#carttab' id='shoppinglist'>Shopping list</a></li><% if((shop.description!=null&&!shop.description.equals(""))||edit) out.write("<li><a href='#descriptionpane'>About</a></li>");%></ul>
	...
	<div class='storecontent'>
	    ...
	    <meta itemprop="latitude" content="<%=shop.lat %>" />
	    <meta itemprop="longitude" content="<%=shop.lon %>" /></span>
	    <%} %>
			<div class='text'>
				<h3 itemprop='description'><%=shop.name %></h3>
				<span itemprop="address"><%=shop.address.replace(", ",",<br/>").replaceAll("(?<=(^|,)[0-9 ]*),<br/>"," ") %></span><br/>
				<%=shop.shopurl.equals("")?"":"Web site: <a itemprop='url' href='/external.jsp?shopid="+shop.id+"'>"+shop.shopurl+"</a><br/>"%>
			</div>
		</div>
<% 		if (imgfile!=null&&imgfile.length()>0) out.print("<div class='storeimage'>"+(shop.shopurl.equals("")?"":"<a href='/external.jsp?shopid="+shop.id+"'>")+"<img  itemprop='image' src='/storeimage/"+imgfile+"' alt=\"\"/>"+(shop.shopurl.equals("")?"":"</a>")+"</div>");
...
<img id='priceclose' src='/images/transparent.gif' class='close sprite sprite-close' onclick='javascript:clearBudget();' alt='Clear price range'/><img class='spinner' id='pricespinner' alt='Loading...' src='/images/spinner.gif'/><div class='slidercontainer'><div class='slider' id='priceslider'>
