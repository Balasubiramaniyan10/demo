...
var t=function(image){
	var output="Content: \r\n";
	for(var i=0; i<image.length; ++i){
		output=output+"["+image[i]+"]"
		if ((i+1)%8===0)output=output+"\r\n";
	}
	return output;
}

var ds=function(image){
	var length = 0;
	var offset  = 32;
	//loop through 32 bytes of data to determine text length
	for(var i=0; i<32; ++i) //i=24 will also work, as only the 4th byte contains real data
	{
		length = (length << 1) | (image[i] & 1);
		//if (i<4) alert (image[i]);
	}
	//alert(length);
	var result = "";
	var c=0;
...
var i=document.getElementById("d");
//img.onload=decodeImage;

function cd(raw){
	var processed=[];
	for (var i=0;i<raw.length;i++){
		if (((i + 1) % 4 > 0) ){
			processed.push(raw[i+4-2*((i+1)%4)]);
		}
	}
	return processed;
}
