...
mapdataset.maptype="G_NORMAL_MAP";
...
map.setMapType(<%=mapdataset.maptype%>);

var mapmarkers= [];
	var reddoticon = new GIcon();
	reddoticon.image = "/images/redmarker.gif";
...
function drawmap(markers){
	...
	map.addControl(new GMapTypeControl());
   	...
   	GEvent.addListener(map, "dragend", function(){refresh();});
    ...
    function loadmap(markers) {
        ...
        sidebar.innerHTML='';
	  	for (var i = 0; i < markers.length; i++) {
			...
			var icon=reddoticon;
			if (markers[i].offers&&markers[i].offers>0) icon=greendoticon;
