...
%><%
	...
	%><%
		...
		for (int i=0;i<alternatives.size();i=i+3){
			...
			int singlevintage=0;
				...
				String ratings=Webroutines.getRatingsHTML(wineset.bestknownwineid,11,"/mobile.jsp",singlevintage)+"";
				if (!ratings.equals("")) {
				out.print(ratings);
	...
	%><h2>Price comparison: <%
		...
		%>
		Click to sort by <%
				out.print("<a href='/mobile.jsp?keepdata=true&amp;offset=0&amp;order="+Webroutines.getOrder(searchdata.getOrder(),"vintage")+"'>Vintage</a>");
			%>, <%
				out.print("<a href='/mobile.jsp?keepdata=true&amp;offset=0&amp;order="+Webroutines.getOrder(searchdata.getOrder(),"size")+"'>Size</a>, ");
			%><%
				out.print("<a href='mobile.jsp?keepdata=true&amp;offset=0&amp;order="+Webroutines.getOrder(searchdata.getOrder(),"priceeuroex")+"'>Price</a>");
	...
	for (int i=0;i<wineset.records;i=i+Webroutines.numberofnormalrows){
	 out.print("<a href='"+response.encodeURL("/mobile.jsp?keepdata=true&amp;offset="+i)+"' >");
	...
	%><div id="note">Note: Prices shown exclude VAT, shipping and handling costs and may exclude duty. Always check the price with the seller.</div><%
		...
		out.print(Webroutines.getTipsHTML("",(float)0.75,20,"mobile.jsp",Translator.languages.EN).replaceAll("&nbsp;","&#160;").replaceAll("&euro;","&#8364;"));
		out.print((Webroutines.getTipsHTML("",(float)0.75,20,"mobile.jsp",Translator.languages.EN)).equals("")?("<br />"+new Translator(Translator.languages.EN).get("notips")):("<br />"+new Translator(Translator.languages.EN).get("pricenote")+"<br />"));
