...
function ooTimeline_setDimension(dimension) {
...
String lastyear=(dateFormat.format(cal.getTime()));
%>
oo.setOOId("eaebe9d1819a45f59d44cb37641cdbb1");

oo.load(function()
{
	//var p= new oo.Pie("12271340", new Date("<%=lastmonth%>"), new Date("<%=now%>"));
	//p.setMetric('ga:visitors', 'Visitors');
	//p.setDimension('ga:country');
	//p.query.setSort('-ga:visitors');
	//p.setOption('title', 'Visitors per country in last month');
	//p.setOption('sliceVisibilityThreshold',2/360);
	//p.setOption('chartArea.width',600);
	//p.setOption('chartArea.height',500);
	//p.setOption('width',600);
	//p.setOption('height',500);
	//p.draw('pie');
	//var tl = new oo.Timeline("12271340", new Date("<%=lastyear%>"), new Date("<%=lastmonth%>"));
	//tl.addMetric('ga:visitors', 'Visits');
	//tl.setOption('colors', ['red', 'orange', 'yellow', 'green']);
	//tl.setOption('title', 'Vinopedia unique visitors per day');
	//tl.setOption('legend','none');
	//tl.draw('timeline');
	
	var query_1 = function(cb){
					var q1 = new oo.Query("12271340", new Date("<%=lastyear%>"), new Date("<%=lastmonth%>"));
 					//Metrics
					q1.addMetric('ga:visitors');
					//Dimension
					q1.addDimension('ga:month');
					q1.execute(function (data1) {				    
						cb(data1);
					});
				};
 
				query_1(function(data1){
					target = (new Date()).getMonth() + 1;
 
					after = data1.splice(0, target);
					...
					prettyDate = function(d){
						...
						return smonth + " " + d.getFullYear();
					};
 
					var j = 0;
					for(var i = data1.length-1; i >= 0; i--){
						d = new Date();
						d.setMonth(d.getMonth() + 1 - j);
						j++;
						data1[i][0] = prettyDate(d);
					}
 
					//Add column labels
					data1.unshift(['Month', 'Visitors']);
 
					//use google visualization (loaded by oocharts already) to draw line chart
