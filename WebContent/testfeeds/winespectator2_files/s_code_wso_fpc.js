...
s.linkInternalFilters="javascript:,winespectator.com,buysub.com,mkt2105.com"
...
s.formList="packaged_order_form_March1,discount_order-071,packaged_order_form_0421091,CustServ,donor_form" //add form names to track
...
function s_doPlugins(s) {
  ...
  /* Internal Campaigns */
	if(!s.eVar4) s.eVar4=s.getQueryParam('icid');

  /* Link Handler - Download Tracking */
	var url=s.downloadLinkHandler();
	if(url){
	  s.eVar7=s.prop7=url.substring(url.lastIndexOf("/")+1,url.length);
	  if(s.pageName) s.prop8=s.pageName;
  ...
  /* Campaign Pathing */
	var getCmp = s.getAndPersistValue(s.campaign,'s_cp_persist',0);
	if (getCmp) s.prop4=getCmp+'-'+s.pageName; //to path all pages with a campaign

  /* Visitor Segmentation (anon if nothing is set) */
	if(s.prop14&&!s.eVar8) s.eVar8=s.prop14;
	if(!s.prop14&&!s.eVar8) s.prop14=s.eVar8="anon";
  ...
  /* Site Search */
	if(s.prop5){
	  ...
	  s.eVar5=s.prop5;
	  var t_search=s.getValOnce(s.eVar5,'ev2',0);
	  if(t_search)s.events=s.apl(s.events,"event5",",",2);
	}
	s.prop11=s.getPreviousValue(s.pageName,'gpv_p7','event5'); //search origination page

  /* Server  */
      s.server = window.location.hostname;
      var e = s.server.split(/\./);
      if(e.length > 1) s.server = e[e.length-2];

  /* Time Parting 2.0 - Eastern Time */
	s.prop12=s.eVar12=s.getTimeParting('h','-5');
	s.prop13=s.eVar13=s.getTimeParting('d','-5');

  /* Page View Event */
	s.events=s.apl(s.events,"event8",",",2);
	
  /* Capture Purchase ID into eVar */
	if(s.purchaseID) s.eVar9=s.purchaseID;
...
/*
 * Plugin: getNewRepeat 1.05 - Return whether user is new or repeat
 */
...
// DV adding code to switch to using First Party Cookies 9/1/2011
s.trackingServer="metrics.winespectator.com";
s.trackingServerSecure="smetrics.winespectator.com";
