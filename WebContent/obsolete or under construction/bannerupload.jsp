...
<%	
	...
	%><html>
	<head>
	<title>Upload banner for <%=Webroutines.getShopNameFromShopId(al.shopid,"")%></title>
...
<%
	  	...
	  	if (fileItemsList.size()>0){
	  		if (fileItemsList.size()>1){
	      		...
	      		if (fileItem.getSize()>200*1024){
	      			out.write("Upload failed. Size of image is "+(int)(fileItem.getSize()/1024)+"kB.Maximum size of images is 200kB.");
	      			fileItem=null;
	...
	"values ("+ii.getWidth()+","+ii.getHeight()+",'<img src=\\'/images/gen/"+bannerid+"."+imagetype+"\\' alt=\\'Banner\\' />','upload','"+link+"',"+al.shopid+","+al.partnerid+",0,'',0.50,0);";
	...
	//close connection
	      			...
	      			logger.logaction();
	      			al.run();
	  				session.setAttribute("message","<b>The uploaded file has been saved successfully.</b><br/>The status of this banner is still set to \"inactive\", which means visitors will not see your banner. In the banner  overview below ypou can activate it.");
	session.removeAttribute("image");
	...
	%>
				...
				<%
					if (check) {
																...
																hideuploaddialog=true;
																if ((ii.getWidth()>160||ii.getHeight()>600)&&(ii.getWidth()>768||ii.getHeight()>90)) {
																	out.write("<b>This image size does not fit any of the banner sizes! Please upload a different image.</b><br/>");  
																	showexample=false;
...
<%@ include file="/snippets/topbar.jsp" %>
		...
		<div id='main'>
			...
			<div id='mainleft'>	
				<%=Webroutines.getTabbedWineResultsHTML(p.s.wineset,p.t,searchdata,25,response,"false",false,p.thispage,false,false)%>
...
al.run();
	...
	<br/><br/>
	Banners can be supplied as an image in the following formats: GIF, JPEG (JPG) or PNG.<br/>
	<br/>
	Banners can be supplied in different sizes. The size determines the placement of the banner. See below for the maximum sizes.<br/><br/>
	<img src='/images/bannersizes.gif' alt='Ads' />
	<%}
	}%>
<%@ include file="snippets/footer.jsp" %>
</body>
