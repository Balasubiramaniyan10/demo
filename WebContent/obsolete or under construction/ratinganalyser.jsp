...
<table class="main" onclick="javascript:emptySuggest();">
	...
	<td class="centre"><%=Webroutines.getConfigKey("systemmessage")%>
		<form action='ratinganalyser.jsp' method="post" id="RegionSearchform" name="Searchform">
		<table class="searchform">
	...
	<%
		...
		for (int i=0;i<advice.wine.length;i++){
			String html="<table>";
			html+="<tr>";
			html+="<td><a href='/wine/"+Webroutines.URLEncode(advice.wine[i].Name+" "+advice.wine[i].Vintage)+"'>"+advice.wine[i].Name+"</a></td>";
			html+="<td><a href='/wine/"+Webroutines.URLEncode(advice.wine[i].Name+" "+advice.wine[i].Vintage)+"'>"+advice.wine[i].Vintage+"</a></td>";
			html+="<td>"+Webroutines.formatPrice(advice.wine[i].PriceEuroIn,(double)0,advice.getCurrency(),"IN")+"</td>";
			html+="<td>"+advice.rating[i]+"</td>";
			html+="</tr>";
			html+="</table>";
			out.write(html);
			out.write("Sources of rating:<br/>");
			rs=Dbutil.selectQuery("Select * from ratings where knownwineid="+advice.wine[i].Knownwineid+" and vintage="+advice.wine[i].Vintage+";",con);
			while (rs.next()){
		String[] terms=rs.getString("name").split("[ '\"()-]");
		for (int j=0;j<terms.length;j++){
			if (!advice.wine[i].Name.toLowerCase().contains(terms[j].toLowerCase())){
		System.out.println(terms[j]);
		String lSQL="update knownwines set fulltextsearch=concat(fulltextsearch,&apos; -"+Spider.SQLEscape(terms[j]).toLowerCase()+"&apos;) where id="+rs.getString("knownwineid")+";";
		out.write("<a href=\"updateknownwine.jsp?sql="+lSQL+"\" target=\"_blank\" onclick=\"mark(this)\">"+terms[j]+"</a> ");
			} else {
		out.write (terms[j]+" ");
			}
		}
		out.write("<br/>");
			...
			out.write("3 cheapest prices:<br/>");
			rs=Dbutil.selectQuery("Select * from wines where knownwineid="+advice.wine[i].Knownwineid+" and vintage="+advice.wine[i].Vintage+" order by priceeuroin limit 3;",con);
			while (rs.next()){
		String[] terms=rs.getString("name").split(" ");
		for (int j=0;j<terms.length;j++){
			String lSQL="update knownwines set fulltextsearch=concat(fulltextsearch,&apos; -"+Spider.SQLEscape(terms[j]).toLowerCase()+"&apos;) where id="+rs.getString("knownwineid")+";";
