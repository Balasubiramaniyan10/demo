...
/**
* The <code>VpHTMLParser</code> class is an HTML DOM parser.
...
*/
public class VpHTMLParser {
private static final Logger logger = Logger.getLogger(VpHTMLParser.class.getName());
...
/**
 * Constructs a <code>VpHTMLParser</code>.
 ...
 */
public VpHTMLParser(Document document, ErrorHandler errorHandler, String publicId, String systemId) {
	this.ucontext = null;
 ...
 * @param systemId The system ID of the document.
 */
public VpHTMLParser(UserAgentContext ucontext, Document document, ErrorHandler errorHandler, String publicId, String systemId) {
 ...
 * @param document A W3C Document instance.
 */
public VpHTMLParser(UserAgentContext ucontext, Document document) {
	this.ucontext = ucontext;
	this.document = document;
			...
			if(tag.startsWith("!")) {
				...
				try {
					...
					parent.appendChild(element);
						...
						Dbutil.logger.info(parent+" "+element);
					}
    				...
    				while((intCh = reader.read()) != -1) {
    					...
    					if(ch == '>') {
    						...
    						if(thisTag.equalsIgnoreCase(tagName)) {
    							...
    							return VpHTMLParser.TOKEN_END_ELEMENT;
    						}
    						else {
    							break INNER;
    						}
			...
			if(attributeName != null) {
				...
				if(attributeValue == null) {
					...
					element.setAttribute(attributeNameStr, actualAttributeValue.toString());
						...
						Dbutil.logger.info(attributeNameStr+" "+actualAttributeValue.toString());
						Dbutil.logger.info(element);
					}
				}
			}
			this.justReadTagBegin = false;
