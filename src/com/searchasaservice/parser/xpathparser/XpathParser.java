...
public class XpathParser implements Serializable{
	...
	*/
	...
	public Result parse() {
		...
		List<HtmlElement> itemnodes= (List<HtmlElement>) document.getByXPath(config.get(0).xpath);
		List<?> n=null;
		Field field;
		for (int i=0;i<itemnodes.size();i++){
			...
			for (AbstractConfigField xpfield:config){
				...
				if (itemfound&&!xpfield.isItemField){
					n=null;
					if (xpfield.xpath.length()>0) n = itemnodes.get(i).getByXPath(xpfield.xpath);
					if (n!=null&&n.size()>0){
						field.setContent(xpfield.getContentHandler().getContent((Node)n.get(0)));
						if (!"".equals(((Element)n.get(0)).getAttribute("fwscounter"))) field.fwscounter=Integer.parseInt(((Element)n.get(0)).getAttribute("fwscounter"));
					...
					if(xpfield.isItemField){
						field=xpfield.getField();
						if (!"".equals(((Element)itemnodes.get(i)).getAttribute("fwscounter"))) field.fwscounter=Integer.parseInt(((Element)itemnodes.get(i)).getAttribute("fwscounter"));
					}
				}
